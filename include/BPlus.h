...
class BPlusTree {
 private:
  ...
  void unlockWQueue(
      std::deque<std::unique_lock<std::shared_mutex> *> &queueLatch) {
    // 释放所有的锁;
    // 释放所有的锁;
    while (!queueLatch.empty()) {
      std::unique_lock<std::shared_mutex> *item = queueLatch.front();
      item->unlock();
      std::cout << "delete Wlock" << std::endl;
      queueLatch.pop_front();
      delete item;
    }
  }

  void unlockRQueue(
      std::deque<std::shared_lock<std::shared_mutex> *> &queueLatch) {
    // 释放所有的锁;
    while (!queueLatch.empty()) {
      std::shared_lock<std::shared_mutex> *item = queueLatch.front();
      item->unlock();
      std::cout << "delete Rlock" << std::endl;
      queueLatch.pop_front();
      delete item;
    }
  }
   ...
   * @return void
   */
  bool doInsert(BPlusTreeNode<KeyType> *&cur, const KeyType &key,
                uint64_t value, BPlusTreeNode<KeyType> *parent, int pos,
                std::deque<std::shared_lock<std::shared_mutex> *> &queueLatch) {
    BPlusTreeNode<KeyType> *&temp = cur;
    int size = temp->keys.size();  // 记录当前需要处理的元素个数
    if (!temp->isLeaf()) {         // 分支节点
      std::shared_lock<std::shared_mutex> *lock =
          new std::shared_lock<std::shared_mutex>(temp->rw_mutex);
      if (!queueLatch.empty()) {
        queueLatch.front()->unlock();
        delete queueLatch.front();
        queueLatch.pop_front();
      }
      ...
      for (decltype(temp->keys.size()) i = 0; i < size; i++) {
        if (key <= temp->keys[i] ||
          ...
          return doInsert(Node, key, value, temp, i, queueLatch);  // 递归查找
        }
      }
    } else {  // 叶子节点
      std::unique_lock<std::shared_mutex> lock(temp->rw_mutex);
      // 大于M个、或增加的值会影响的父节点，不安全
      if (temp->keys.size() + 1 > MAX_KEYS ||
          (parent != nullptr && key > temp->keys.back())) {
  ...
  void doInsertPessmistic(
      BPlusTreeNode<KeyType> *&root, const KeyType &key, uint64_t value,
      BPlusTreeNode<KeyType> *parent, int pos,
      std::deque<std::unique_lock<std::shared_mutex> *> &queueLatch) {
    BPlusTreeNode<KeyType> *&temp = root;
    int size = temp->keys.size();  // 记录当前需要处理的元素个数
    std::unique_lock<std::shared_mutex> *lock =
        new std::unique_lock<std::shared_mutex>(temp->rw_mutex);  // 加写锁
    queueLatch.push_back(lock);
    if (!temp->isLeaf()) {  // 分支节点
      for (decltype(temp->keys.size()) i = 0; i < size; i++) {
        if (key <= temp->keys[i] ||
          ...
          auto Node = static_cast<InternalNode<KeyType> *>(temp)->children[i];
          doInsertPessmistic(Node, key, value, temp, i,
                             queueLatch);      // 递归查找
          if (temp->keys.size() > MAX_KEYS) {  // 需要分裂
            splitInternalNode(parent, pos, temp);
          } else {
            unlockWQueue(queueLatch);
          }
          if (parent == nullptr) {  // 到了根节点
            unlockWQueue(queueLatch);
          }
          return;
        }
      ...
      if (leafNode->keys.size() > MAX_KEYS) {
        ...
        // 重组
        if (parent == nullptr) {  // 非叶根节点至少有两个元素, 创建新父节点
   ...
   * @return void
   */
  void borrowOrMergeLeafNode(
      BPlusTreeNode<KeyType> *&root, InternalNode<KeyType> *parent, int pos,
      KeyType key,
      std::deque<std::unique_lock<std::shared_mutex> *> &queueLatch) {
    auto index = static_cast<LeafNode<KeyType> *>(root);
    if (parent == nullptr) {  // 叶根节点，不处理
      if (index->keys.size() == 0) {
        std::unique_lock<std::shared_mutex> lock(index->pre->rw_mutex);
        index->pre->next = nullptr;
        index->pre = nullptr;
        unlockWQueue(queueLatch);  // 因为这里是最后的位置
        delete index;
        root = nullptr;
        index = nullptr;
        return;
   ...
   * @return void
   */
  void borrowOrMergeInternalNode(
      BPlusTreeNode<KeyType> *&internalNode, InternalNode<KeyType> *parent,
      int pos, std::deque<std::unique_lock<std::shared_mutex> *> &queueLatch) {
    auto index = static_cast<InternalNode<KeyType> *>(internalNode);
    if (parent == nullptr) {  // 不会出现当前节点是根叶节点进来 ;
   ...
   * @return void
   */
  bool doRemove(std::vector<uint64_t> &result, BPlusTreeNode<KeyType> *root,
    ...
    if (!temp->isLeaf()) {  // 分支节点
      for (decltype(temp->keys.size()) i = 0; i < size; i++) {
        if (key <= temp->keys[i] || i == (size - 1)) {  // 第一个小或到最后
          ...
          auto Node = static_cast<InternalNode<KeyType> *>(temp)->children[i];
          return doRemove(result, Node, key, temp, i, queueLatch);  // 递归查找
        }
      }
    } else {
      // 叶子节点
      ...
      // 删除元素
      std::unique_lock<std::shared_mutex> lock(leafNode->rw_mutex);
  ...
  void doRemovePessmistic(
      std::vector<uint64_t> &result, BPlusTreeNode<KeyType> *root,
      const KeyType &key, BPlusTreeNode<KeyType> *parent, int pos,
      std::deque<std::unique_lock<std::shared_mutex> *> &queueLatch) {
    if (root == nullptr) {
      std::cout << "the root is nullptr." << std::endl;
    ...
    if (!temp->isLeaf()) {         // 分支节点
      for (decltype(temp->keys.size()) i = 0; i < size; i++) {
        if (key <= temp->keys[i] || i == (size - 1)) {  // 第一个小或到最后
          ...
          auto Node = static_cast<InternalNode<KeyType> *>(temp)->children[i];
          doRemovePessmistic(result, Node, key, temp, i,
                             queueLatch);  // 递归查找
          ...
          if (temp->keys.size() < (MAX_KEYS + 1) / 2) {
            borrowOrMergeInternalNode(temp,
                                      static_cast<InternalNode<int> *>(parent),
                                      pos, queueLatch);
          } else {
            unlockWQueue(queueLatch);
          }
  ...
  void doSearch(std::vector<uint64_t> &result, BPlusTreeNode<KeyType> *index,
                const KeyType &key,
                std::deque<std::shared_lock<std::shared_mutex> *> &queue) {
    int size = index->keys.size();  // 记录当前需要处理的元素个数
    std::shared_lock<std::shared_mutex> *lock =
        new std::shared_lock<std::shared_mutex>(index->rw_mutex);
    if (!queue.empty()) {
      queue.front()->unlock();
      delete queue.front();
      queue.pop_front();
    ...
    for (decltype(index->keys.size()) i = 0; i < size; i++) {
      if (!index->isLeaf()) {         // 分支节点
        if (key <= index->keys[i]) {  // 第一个小或到最后
          ...
          doSearch(result, Node, key, queue);  // 递归查找
          if (!queue.empty()) {
            unlockRQueue(queue);
          }
          return;
 ...
 public:
  ...
  void insert(const KeyType &key, uint64_t value, int flag = 0) {
    if (root == nullptr) {
      ...
      std::deque<std::shared_lock<std::shared_mutex> *> queueLatch;
      if (!doInsert(root, key, value, nullptr, 0, queueLatch)) {
        if (!queueLatch.empty()) {
          std::cout << "dont delete Rlock" << std::endl;
          unlockRQueue(queueLatch);
        }
        BPlusTreeNode<KeyType> *cur = root;
        std::deque<std::unique_lock<std::shared_mutex> *> queueWLatch;
        doInsertPessmistic(root, key, value, nullptr, 0, queueWLatch);
        if (!queueWLatch.empty()) {
          std::cout << "dont delete Wlock" << std::endl;
          unlockWQueue(queueWLatch);
        }
      }
    }
  }
  ...
  void remove(std::vector<uint64_t> &result, KeyType key) {
    if (root == nullptr) {
      std::cout << "the tree is empty, cant delete any element." << std::endl;
      return;
    }
    std::deque<std::shared_lock<std::shared_mutex> *> queueLatch;
    if (!doRemove(result, root, key, nullptr, 0, queueLatch)) {
      if (!queueLatch.empty()) {
        unlockRQueue(queueLatch);
      }
      std::deque<std::unique_lock<std::shared_mutex> *> queueWLatch;
      doRemovePessmistic(result, root, key, nullptr, 0, queueWLatch);
      if (!queueWLatch.empty()) {
        unlockWQueue(queueWLatch);
      }
    }
   ...
   * @return void
   */
  void search(const KeyType &key, std::vector<uint64_t> &res) {
    std::deque<std::shared_lock<std::shared_mutex> *> queue;
    doSearch(res, root, key, queue);
  }
